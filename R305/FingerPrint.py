    ...
    def search(self):
        ...
        if(x==0):
          ...
          return list(d)[6]
            ...
            print(d)
            return -1
    def searchapi(self,location ,amount ,posid):
        data=[0x00,0x08,0x04,0x01,0x00,0x00,0x00,0x10]
        self.writepac(data)
        d=self.readpac()
        x=list(d)[4]
        if(x==0):
          ...
          return list(d)[6]
            ...
            try:
              payload = {'location': location ,'merchant_id':posid,'card_no':amount,'city':location}
              r = requests.post("http://www.pksystems.co.zw/fingerprint/public/reports/pyapi", data=payload)
              result = r.json()
              print (result["save"])
